...
import './live_feed_post_submit.html';
...
Template.liveFeedPostSubmit.events({
	...
	'click .post-submit--button-delete-file': function(e) {
		...
		if (confirm(TAPi18n.__('post-submit--confirm-delete-file'))) {
			...
			var fileIdElement = Files.findOne({_id:fileId});
			if (fileId) {
				Files.remove(fileIdElement._id), function(err){console.log(err)}
				Meteor.call('deleteFile',fileIdElement);
			}
		}
		$('.resources-post-submit--submit').prop('disabled', true);
	}
});
